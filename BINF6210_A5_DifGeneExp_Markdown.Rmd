...
knitr::opts_chunk$set(tidy.opts=list(width.cutoff=88))
```
...
#Fix labels and other graph details. Doesn't matter which count object you put in here, either will produce the same
DS_AveLogCPM <- aveLogCPM(DS_Counts)
hist(DS_AveLogCPM)
```
...
``` {r de-orig, message = FALSE, warning = FALSE}
