...
df_top_p <- unique(c(top_p_DS, top_p_ER, top_p_OG))
df_top_p <- data.frame(Gene = df_top_p, DESeq2 = rep("DESeq2", 19), EdgeR = rep("EdgeR", 19), Original = rep("Original", 19))
df_top_p[!(df_top_p$Gene %in% top_p_DS), "DESeq2"] <- NA
df_top_p[!(df_top_p$Gene %in% top_p_ER), "EdgeR"] <- NA
df_top_p[!(df_top_p$Gene %in% top_p_OG), "Original"] <- NA
rownames(df_top_p) <- df_top_p$Gene
df_top_p$Gene = NULL

# By sampling the count matrix that with VS transformation, the desired genes can be expressed as a heatmap
mat <- assay(vst_Counts)[rownames(df_top_p), ]
mat <- mat - rowMeans(mat)
anno <-colData(vst_Counts)
anno <- anno$Group
anno <- data.frame(anno)
rownames(anno) <- colnames(mat)
pheatmap(mat, main = "Heatmap of genes with most significant p-values",
         annotation_col = anno,
